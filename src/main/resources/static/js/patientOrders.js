    ...
    function fetchOrders() {
        $.ajax({
            ...
            success: function(result) {
                ...
                {
                    newstr+="<div class='card w-75 shadow-lg rounded-2 text-black font-weight-bold col-15'> <div class='card-body'> <ul class='list-group'> <li class='list-group-item list-group-item-info' ><i class='fa fa-dollar' style='margin-right: .2rem'>Order ID:</i>"+resultJSON["result"][i]["orderId"]+"</li> <li class='list-group-item list-group-item-info' ><i class='fa fa-sort-amount-asc'  style='margin-right: .2rem' >Order Total:</i>"+resultJSON["result"][i]["totalAmount"]+"</li> <li class='list-group-item list-group-item-info'><i class='fa fa-question-circle' style='margin-right: .2rem'>Order Status:</i>"+resultJSON["result"][i]["orderStatus"]+"</li> </ul> </div> <div class='card-footer"+i+"'> <button type='button' id='show_details'  class='btn btn-outline-info font-weight-bold' aria-expanded='false' aria-controls='collapseId"+i+"' data-bs-toggle='collapse' href='#collapseId"+i+"'>Show Items</button> </div> </div>";
                }
